...
We'll define a common color palette, with one color for each method we evaluated. 

```{r}
palette_methods <- paletteer_d("RColorBrewer::Paired")[c(6, 1, 3, 2, 4, 5, 9, 7, 8)]
```
...
p3a <- ggplot(sim_results, aes(x = method, y = sil, fill = method)) + 
       ...
       scale_y_continuous(labels = scales::label_number(accuracy = .1), 
                          breaks = seq(round(min(sim_results$sil)), round(max(sim_results$sil)), by = .3)) + 
       scale_fill_manual(values = palette_methods) + 
       theme_classic(base_size = 18)  + 
       theme(axis.text.x = element_text(angle = 45, 
                                        hjust = 0.9, 
...
p4 <- sim_results %>% 
      ...
      scale_fill_manual(palette_methods) + 
...
sim_panc_ncell1000_nclust7$cellPopulation2 <- as.integer(sim_panc_ncell1000_nclust7@meta.data$cellPopulation) - 1L
...
fig_save(p0a, plot.name = "Total_ARI_By_Method_And_Reference.pdf", dims = c(9, 9))
fig_save(p0b, plot.name = "Total_ARI_By_Method.pdf", dims = c(8, 8))
fig_save(p1a, plot.name = "Total_NMI_By_Method_And_Reference.pdf", dims = c(9, 9))
fig_save(p1b, plot.name = "Total_NMI_By_Method.pdf", dims = c(8, 8))
fig_save(p2a, plot.name = "Total_DB_Index_By_Method_And_Reference.pdf", dims = c(9, 9))
fig_save(p2b, plot.name = "Total_DB_Index_By_Method.pdf", dims = c(8, 8))
fig_save(p3a, plot.name = "Total_Silhouette_By_Method_And_Reference.pdf", dims = c(9, 9))
fig_save(p3b, plot.name = "Total_Silhouette_By_Method.pdf", dims = c(8, 8))
...
fig_save(p8, plot.name = "Case_Study_True_Label_tSNE.pdf", dims = c(5, 5))
fig_save(p9, plot.name = "Case_Study_Estimated_Clusters_All_tSNE.pdf", dims = c(9, 5))
fig_save(p10a, plot.name = "Case_Study_Estimated_Clusters_Performance.pdf", dims = c(12, 8))
fig_save(p10b, plot.name = "Case_Study_Louvain_Resolution_Performance.pdf", dims = c(8, 6))
fig_save(p10c, plot.name = "Case_Study_Louvain_Resolution_Performance_with_Table.pdf", dims = c(10, 9))
# runtime 
fig_save(p11a, plot.name = "Runtime_Log_By_Method_And_Reference.pdf", dims = c(8, 8))
fig_save(p11b, plot.name = "Runtime_Raw_By_Method_And_Number_Cells.pdf", dims = c(8, 8))
```
