...
p2a <- ggplot(sim_results, aes(x = method, y = db, fill = method)) + 
       ...
       scale_y_continuous(labels = scales::label_number(accuracy = 0.1), 
                          breaks = seq(0, round(max(sim_results$db)), by = 0.25)) + 
       scale_fill_paletteer_d("ggsci::nrc_npg") + 
       theme_classic(base_size = 14)  + 
       theme(axis.text.x = element_text(angle = 45, hjust = 0.9, vjust = 0.9), 
             axis.title.x = element_blank(), 
...
Lastly, we'll run CellSIUS. We first generate a broad clustering, then subcluster using MCL. 
...
if (all(is.na(cellsius_res))) {
  ...
  cellsius_clust <- CellSIUS::CellSIUS_final_cluster_assignment(cellsius_res, group_id = sim_panc_ncell1000_nclust7$seurat_clusters)
  cellsius_clust <- as.factor(cellsius_clust)
}
```
...
ari_res <- data.frame(ARI = c(mclust::adjustedRandIndex(sim_panc_ncell1000_nclust7$cellPopulation, SCISSORS_clusts), 
                      ...
                      Method = c("SCISSORS", "Louvain", "K-means", "Hierarchical", "DBSCAN", "GiniClust3", "CellSIUS")) %>% 
           arrange(desc(ARI)) %>% 
...
sil_res <- data.frame(Sil = c(mean(cluster::silhouette(SCISSORS::CosineDist(Embeddings(sim_panc_ncell1000_nclust7, "pca")[, 1:20]), x = SCISSORS_clusts)[, 3]), 
                              ...
                              mean(cluster::silhouette(SCISSORS::CosineDist(Embeddings(sim_panc_ncell1000_nclust7, "pca")[, 1:20]), x = as.integer(cellsius_clusts))[, 3])), 
                      Method = c("SCISSORS", "Louvain", "K-means", "Hierarchical", "DBSCAN", "GiniClust3", "CellSIUS")) %>% 
           arrange(desc(Sil)) %>% 
...
ari_table <- gridExtra::tableGrob(ari_res,
                                  ...
                                  rows = NULL)
nmi_table <- gridExtra::tableGrob(nmi_res,
                                  cols = c("Method", "Norm. M.I.", "Rank"), 
                                  theme = gridExtra::ttheme_minimal(colhead = list(bg_params = list(fill = "grey90"))), 
                                  rows = NULL)
sil_table <- gridExtra::tableGrob(sil_res,
                                  cols = c("Method", "Mean Sil. Score", "Rank"), 
                                  theme = gridExtra::ttheme_minimal(colhead = list(bg_params = list(fill = "grey90"))), 
                                  rows = NULL)
metric_res <- ari_res %>% 
...
metric_table <- gridExtra::tableGrob(metric_res,
                                     cols = c("Method", "ARI", "ARI Rank", "NMI", "NMI Rank", "Silhouette", "Silhouette Rank", "Overall Rank"), 
                                     theme = gridExtra::ttheme_minimal(colhead = list(bg_params = list(fill = "grey90"))), 
                                     rows = NULL)
```
...
p10a <- (p9 / (((p8 + facet_wrap(~"Ground Truth")) | metric_table))) + 
...
Louvain_res_incr <- purrr::map(c(.5, .7, .9, 1.1, 1.5, 2), function(x) {
  ...
  nmi_val <- aricode::NMI(cluster_res$seurat_clusters, sim_panc_ncell1000_nclust7$cellPopulation)
...
louvain_ari_table <- gridExtra::tableGrob(louvain_metric_res,
                                          cols = c("Resolution", "Adj. Rand Index", "Norm. M.I.", "Sil."), 
                                          theme = gridExtra::ttheme_minimal(colhead = list(bg_params = list(fill = "grey90"))), 
                                          rows = NULL)
```
...
p10b <- ggplot(res_df, aes(x = tSNE_1, y = tSNE_2, color = clust)) + 
        ...
        theme(axis.ticks = element_blank(), 
              ...
              legend.title = element_text(face = "bold")) + 
        guides(color = guide_legend(override.aes = list(size = 3, alpha = 1)))
(p10b / (p8 | louvain_ari_table)) + plot_layout(heights = c(2, 1))      
```
...
fig_save(p2a, plot.name = "Total_DB_Index_By_Method_And_Reference.pdf", dims = c(10, 6))
fig_save(p2b, plot.name = "Total_DB_Index_By_Method.pdf", dims = c(10, 6))
fig_save(p3a, plot.name = "Total_Silhouette_By_Method_And_Reference.pdf", dims = c(10, 6))
fig_save(p3b, plot.name = "Total_Silhouette_By_Method.pdf", dims = c(10, 6))
# top 50% of metric observations
fig_save(p4, plot.name = "Above_Median_ARI_By_Method_And_Reference.pdf", dims = c(10, 6))
fig_save(p5, plot.name = "Above_Median_NMI_By_Method_And_Reference.pdf", dims = c(10, 6))
fig_save(p6, plot.name = "Above_Median_DB_Index_By_Method_And_Reference.pdf", dims = c(10, 6))
fig_save(p7, plot.name = "Above_Median_Silhouette_By_Method_And_Reference.pdf", dims = c(10, 6))
# clustering case study
fig_save(p8, plot.name = "Case_Study_True_Label_tSNE.pdf", dims = c(8, 4.5))
fig_save(p9, plot.name = "Case_Study_Estimated_Clusters_All_tSNE.pdf", dims = c(12, 7))
fig_save(p10a, plot.name = "Case_Study_Estimated_Clusters_Performance.pdf", dims = c(15, 8))
fig_save(p10b, plot.name = "Case_Study_Louvain_Resolution_Performance.pdf", dims = c(15, 8))
# runtime 
fig_save(p11a, plot.name = "Runtime_Log_By_Method_And_Reference.pdf", dims = c(12, 7))
fig_save(p11b, plot.name = "Runtime_Raw_By_Method_And_Number_Cells.pdf", dims = c(12, 7))
```
